...
> 以下为旧分类→新维度的映射统计（部分接口因跨维度交叉而计入多项，映射表条目数与最终分析文档的条目计数不同）。
> **最终接口计数以 `kvm_linux_interface_analysis.md` 统计表为准**：L1=45, L2=35, L3=7, Total=87。

| 旧分类 | 条目数 | 新维度 | 条目数（映射） |
|--------|--------|--------|--------|
| 内存管理 | 17 | 维度 1：内存管理 | 22 |
| 时钟 | 16 | 维度 2：vCPU 管理 | 21 |
| 主机信息 | 8 | 维度 3：I/O 模型 | 2 |
| 虚拟机管理 | 15 | 维度 4：中断与事件 | 11 |
| 架构相关 | 19 | 维度 5：时钟与定时器 | 14 |
| **总计** | **75** | 维度 6：调度与同步 | 17 |
| | | 维度 7：安全与隔离 | 1 |
| | | **总计** | **88**（含交叉计数） |

> 最终分析文档满足 AC-1：L1=45 ✅ (≥15) | L2=35 ✅ (≥35) | L3=7
