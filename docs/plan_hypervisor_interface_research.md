...
每个接口条目应**覆盖**以下 9 个方面的信息，但不强制使用统一的显式字段标签。可使用自然段落、代码块附带说明、或项目符号等任意格式，只要内容覆盖到位即可：

1. 函数签名（或代码块中体现） 2. 源码位置 3. 语义说明 4. 调用示例（代码块或文字描述）
5. 所属 OS 子系统 6. 调用上下文（provider/consumer 视角） 7. 是否 hot path
8. 边界类型（`kernel KPI` / `ioctl UAPI` / `syscall` / `libc wrapper` / `vmm ioctl` / `其他`）
9. 接口契约（输入输出约束、错误码、生命周期、锁/睡眠约束、权限要求、版本稳定性、可替代性）

**判定标准**：审阅时检查内容是否覆盖以上 9 方面，不需要逐条检查显式标签。代码块中的函数签名、代码块后的语义描述、及上下文中的调用关系均可作为覆盖证据。
...
- AC-1: KVM 接口分析文档按 7 大维度重构，不少于 15 个一级接口 + 35 个二级接口，每个条目覆盖 9 方面内容（格式灵活，不强制显式标签）
  - Positive Tests:
    - 每个接口条目覆盖：函数签名（或蕴含于代码块）、源码位置、语义说明、调用示例、所属 OS 子系统、调用上下文、hot path 标记、边界类型、接口契约
    - 文档尾部包含接口层次总览图
